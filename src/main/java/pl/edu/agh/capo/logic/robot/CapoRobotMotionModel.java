...
public class CapoRobotMotionModel {
    ...
    public Location getLocationAfterTime(Measure measure, double deltaTimeInMillis) {
        ...
        double deltaTime = deltaTimeInMillis / 1000;

        if (velocitiesNeedCorrection(velocityLeft, velocityRight, deltaTime)) {
            double divider = Math.max(Math.abs(velocityLeft), Math.abs(velocityRight)) / CapoRobotConstants.MAX_LINEAR_VELOCITY;
    ...
    public void applyLocation(Location location, Measure measure) {
    ...
    private boolean velocityExceedMax(double linearVelocity) {
        return linearVelocity > CapoRobotConstants.MAX_LINEAR_VELOCITY;
    }

    private boolean accelerationExceedMax(double linearVelocity, double deltaTime) {
        double acceleration = (getLinearVelocity() - linearVelocity) / deltaTime;
