package pl.edu.agh.capo.hough.visualization;
    ...
    public void draw(BufferedImage image, int size, int color) {
        ...
        if (theta < Math.PI * 0.25 || theta > Math.PI * 0.75) {
            ...
            for (int y = 0; y < size; y++) {
                int x = (int) ((((rho - houghHeight) - ((y - centerY) * tsin)) / tcos) + centerX);
                if (x < size && x >= 0) {
            ...
            for (int x = 0; x < size; x++) {
                int y = (int) ((((rho - houghHeight) - ((x - centerX) * tcos)) / tsin) + centerY);
                if (y < size && y >= 0) {
                    image.setRGB(x, y, color);
                }
            }
